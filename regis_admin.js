...
form.onsubmit = function (event) {
    ...
    var fileInput = document.getElementById("affiliation_proof");
    var guidelines = document.getElementById("guidelines").checked;
    ...
    if (!adminIdRegex.test(admin_id)) {
        alert("Admin ID must be 'ADM' followed by exactly 5 digits (e.g. ADM12345).");
    ...
    if (age < 18 || age > 100) {
        alert("Admin must be at least 18 years old and age must be valid.");
        document.getElementById("dob").focus();
        return false;
    }
    ...
    if (fileInput.files.length === 0) {
        alert("Please upload Proof of Affiliation.");
        fileInput.focus();
        return false;
    }

    if (!guidelines) {
        alert("You must agree to the Community Guidelines & Code of Conduct.");
        document.getElementById("guidelines").focus();
        return false;
    }

    showPopup("Admin registration successful!");
    return true;
};
